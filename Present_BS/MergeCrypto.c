...
static void sbox_and_pbox(bs_reg_t state[CRYPTO_IN_SIZE_BIT])
{
    ...
    for (bsPos = 0; bsPos < CRYPTO_IN_SIZE_BIT; bsPos += 4) {
        ...
        state_out[perPos] = (state[bsPos] & state[bsPos + 2] & state[bsPos + 1]) ^ (state[bsPos] & state[bsPos + 3] & state[bsPos + 1]) ^ (state[bsPos + 3] & state[bsPos + 1]) ^ state[bsPos + 1] ^ (state[bsPos] & state[bsPos + 2] & state[bsPos + 3]) ^
                     (state[bsPos + 2] & state[bsPos + 3]) ^ state[bsPos + 3];
        perPos = (bsPos+2) / 4 + (bsPos+2) % 4 * 16;
        state_out[perPos] = ~((state[bsPos] & state[bsPos + 1]) ^ (state[bsPos] & state[bsPos + 3] & state[bsPos + 1]) ^ (state[bsPos + 3] & state[bsPos + 1]) ^
        ...
        state_out[perPos] = ~((state[bsPos + 1] & state[bsPos + 2] & state[bsPos]) ^ (state[bsPos + 1] & state[bsPos + 3] & state[bsPos]) ^
